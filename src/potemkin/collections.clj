...
(defmacro def-tuple [name cardinality]
  (let [fields (map
                 #(symbol (str "e" %))
                 (range cardinality))]
    (unify-gensyms
...
(defn tuple
  "Returns tuple structures that cannot be changed (even via immutable operators), which are highly efficient
   for index lookups and hash calculations.  If the arity is greater than 4, defaults to returning a vector."
  ([]
     (Tuple0.))
  ([x]
     (Tuple1. x))
